...
impl JwtConfig {
    pub fn from_env() -> Result<Self, AppError> {
         let secret = std::env::var("JWT_SECRET")
            .map_err(|_| {
                AppError::Internal(
                    "JWT_SECRET 환경변수가 설정되지 않았습니다.".into()
                )
            })?;
        ...
        let access_token_expires_in_seconds =
            std::env::var("JWT_ACCESS_TOKEN_EXPIRES_IN_SECONDS")
    ...
    // Access Token 생성
    pub fn create_access_token(&self, account_id: i64, roles: Vec<AccountRole>) -> Result<String, AppError> {
        // 1. 현재 시간으로 iat 설정
        let now = Utc::now();
...
mod tests {
    ...
    fn test_config() -> JwtConfig {
        JwtConfig::new(
            "test-secret-key-for-unit-test-only",
            3600,
        )
    }
    ...
    fn verify_invalid_token_fails() {
        ...
        let result =
            config.verify_access_token("invalid.jwt.token");
    ...
    fn verify_wrong_secret_fails() {
        ...
        let wrong_config = JwtConfig::new(
            "completely-different-secret",
            3600,
        );

        let result =
            wrong_config.verify_access_token(&token);
