...
async fn main() {
    ...
    // 로깅 설정
    if std::env::var("RUST_LOG").is_err() { // RUST_LOG 환경변수가 세팅되어 있지 않다면 수동 설정
        unsafe { 
    ...
    let redis_client = RedisClient::new(redis).await.expect("Redis/Dragonfly 연결 초기화 실패");
    redis_client.ping().await.expect("Redis/Dragonfly 연결 확인 실패");

    tracing::info!("[CACHE CONNECTED] {} 연결 성공", cache_backend);

    // AppState
    let state = AppState { postgres: db_pool, jwt, redis: redis_client, }; 
