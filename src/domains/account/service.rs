...
use crate::common::security::password::{hash_password, verify_password};
use crate::common::{
    auth::{
        jwt::JwtConfig,
        refresh_token::{
            delete_refresh_token, find_refresh_token, generate_refresh_token, rotate_refresh_token,
            save_refresh_token,
        },
        role::AccountRole,
    },
...
use crate::domains::account::{
    dto::{AccountResponseDto, CreateAccountDto, LoginAccountDto},
    repository::{AccountRepository, CreateAccountParams},
};
...
impl AccountService {
    ...
    pub async fn create_account(&self, payload: &CreateAccountDto) -> Result<(), AppError> {
        ...
        let params = CreateAccountParams::new(email, password_hash, name);

        let mut tx = self.account_repository.begin().await?;

        let account_id = self.account_repository.save(&mut tx, params).await?;

        self.account_repository
            .assign_role(&mut tx, account_id, &role)
            .await?;
    ...
    pub async fn login(&self, payload: &LoginAccountDto) -> Result<TokenResult, AppError> {
        ...
        let password_valid = verify_password(&payload.password, &account.password_hash)?;
        ...
        // 6. Access Token 발급
        let access_token = self.jwt.create_access_token(account.id, roles)?;

        // 7. Refresh Token 발급 및 Redis에 갱신
        let refresh_token = generate_refresh_token()?;

        save_refresh_token(&self.redis, &refresh_token, account.id).await?;
    ...
    */
    pub async fn refresh(
        ...
        let claims = self.jwt.verify_expired_access_token(access_token)?;
        ...
        // 6. 새 Access Token 발급
        let access_token = self.jwt.create_access_token(refresh_account_id, roles)?;
